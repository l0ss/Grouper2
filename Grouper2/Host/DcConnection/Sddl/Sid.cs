...
namespace Grouper2.Host.DcConnection.Sddl
{
    ...
    public class SddlSid
    {
        [JsonProperty("Raw")] public string Raw { get; }

        [JsonProperty("Alias")] public string Alias { get; }
        
        public SddlSid(string sid)
        {
            Raw = sid;
            ...
            if (alias == null)
            {
                alias = Match.OneByPrefix(sid, KnownAliasesDict, out _);
            }

            if (alias == null)
            {
                
                // pull down the netconn to try something else
                alias = Ldap.Use().GetUserFromSid(sid);
                if (alias == null)
                {
                    // ERROR Unknown SID.
