...
namespace Grouper2.Auditor
{
    ...
    {
        public JObject AssessGroupMembership(JToken parsedGrpMemberships)
        {
            if (parsedGrpMemberships == null) 
                throw new ArgumentNullException(nameof(parsedGrpMemberships));
            // base interest level
            int interestLevel = 4;
            ...
            foreach (JToken parsedGrpMembership in parsedGrpMembershipsEnumerable)
            {
                ...
                Sid checkedSid = Sid.CheckSid(cleanSid);
                string displayName = "";
                // if we're online, try to look up the Key SID
                if (this._netconn.CanSendTraffic)
                {
                    displayName = this._netconn.GetUserFromSid(cleanSid);
                }
                // otherwise try to get it from the well known sid data
                else if (checkedSid != null)
                {
                    displayName = checkedSid.Name;
                }
                else
                {
                ...
                if (this.InterestLevel <= interestLevel)
                {
        ...
        public JProperty AssessGrpMemberItem(string rawMember)
        {
            ...
            if (rawMember.StartsWith("*"))
            {
                ...
                {
                    // look it up
                    memberDisplayName = this._netconn.GetUserFromSid(memberSid);
                }
                else
                {
                    // see if it's well known
                    Sid checkedMemberSid = Sid.CheckSid(memberSid);
                    if (checkedMemberSid != null)
                    {
                        memberDisplayName = checkedMemberSid.Name;
                    }
                }
            }
